...
public final class VstBroadcastManager {
    ...
    private final HashMap<BroadcastReceiver, Handler> mHandlers
	= new HashMap<BroadcastReceiver, Handler>();
    ...
    public void registerReceiver(BroadcastReceiver receiver, IntentFilter filter, Looper looper) {
		...
		if (looper ==  null) {
		    looper = mAppContext.getMainLooper();
		}
		Handler handler = new Handler(looper) {
                ...
                mHandlers.put(receiver, handler);
            }
            filters.add(filter);
            ...
            if (entries != null) {
                ...
                if (receivers != null) {
                    ...
                    for (Handler handler: mHandlers.values()) {
                        if (!handler.hasMessages(MSG_EXEC_PENDING_BROADCASTS)) {
                            handler.sendEmptyMessage(MSG_EXEC_PENDING_BROADCASTS);
                        }
                    }
     ...
     * returning, i.e. runs on the calling thread.
     */
    ...
    /**
     * this runs on the calling thread called above --it is synchronous!
     */
